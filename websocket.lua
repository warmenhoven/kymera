...
function WebSocket:Close ()
	...
	self.ClosingTimer = TIMER.set(self.ClosingTimer, 3 * ONE_SECOND, _timer)
...
function WebSocket:ConnectionChanged (strStatus)
	...
	self.PingTimer = TIMER.cancel(self.PingTimer)
	if (self.connected) then
		...
		self.PingTimer = TIMER.set(self.PingTimer, self.ping_interval * ONE_SECOND, _timer, true)
